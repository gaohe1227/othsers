...
<div class="body-box">
<div class="rhead">
	<div class="rpos">当前位置: 广告版位 - 列表</div>
	<form class="ropt">
...
<c:forEach items="${advertisingSpaceList}" var="advertisingSpace">
<tr>
	<td><input name="ids" value="${advertisingSpace.adspaceId}" type="checkbox"></td>
	<td>${advertisingSpace.adspaceId}</td>
	<td>${advertisingSpace.adName}</td>
	<td align="center"><c:choose><c:when test="${advertisingSpace.isEnabled==1}">是</c:when><c:otherwise>否</c:otherwise></c:choose></td>
	<td align="center">		<a href="<%=basePath %>/admin/assistant/advertising_space/v_edit.do?id=${advertisingSpace.adspaceId}" class="pn-opt">修改</a> | <a href="<%=basePath %>/admin/assistant/advertising_space/o_del.do?ids=${advertisingSpace.adspaceId}" onclick="if(!confirm('您确定删除吗？')) {return false;}" class="pn-opt">删除</a></td>
</tr>
</c:forEach>
