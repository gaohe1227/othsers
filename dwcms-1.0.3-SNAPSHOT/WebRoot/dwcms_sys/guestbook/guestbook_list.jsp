...
<body><div style="position: absolute; z-index: 19700; top: -1970px; left: -1970px; display: none;"><iframe style="width: 186px; height: 199px;" src="leave_message_list_files/My97DatePicker.html" border="0" frameborder="0" scrolling="no"></iframe></div>
<div class="body-box">
<div class="rhead">
	<div class="rpos">当前位置: 留言管理 - 列表</div>
	<form class="ropt">
...
<c:forEach items="${guestbookDtoList}" var="guestbookDto">
<tr>
	<td><input name="ids" value="${guestbookDto.guestbookId}" type="checkbox"></td>
	<td>${guestbookDto.guestbookId}</td>
	<td>${guestbookDto.title}</td>
	<td align="center">admin</td>
	<td align="center"><fmt:formatDate value="${guestbookDto.createTime}"  type="date" pattern="yyyy-MM-dd HH:mm:ss"/> </td>
	<td align="center">
	<c:choose>
		<c:when test="${guestbookDto.isRecommend}">
			是
		</c:when>
		<c:otherwise>
			否
	...
	<td align="center">
	<c:choose>
		<c:when test="${guestbookDto.isChecked}">
			是
		</c:when>
		<c:otherwise>
			否
	...
	<td align="center">		<a href="<%=basePath %>/admin/assistant/guestbook/v_edit.do?id=${guestbookDto.guestbookId}&amp;pageNo=${page.currentPage}" class="pn-opt">修改</a> | <a href="<%=basePath %>/admin/assistant/guestbook/o_del.do?ids=${guestbookDto.guestbookId}&amp;pageNo=${page.currentPage}" onclick="if(!confirm('您确定删除吗？')) {return false;}" class="pn-opt">删除</a></td>
</tr>
</c:forEach>
</tbody>
</table>
