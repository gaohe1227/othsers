...
<tr>
	...
	<td align="center">		<a href="<%=basePath %>admin/core/res/v_rename.do?root=${root}&name=%2F${file.filename}" class="pn-opt">重命名</a> | 	<c:choose><c:when test="${file.isEditable}"><a  href="<%=basePath %>/admin/core/res/v_edit.do?root=${root}&filename=${file.filename}" class="pn-opt">修改</a></c:when><c:otherwise><a  disabled="disabled">修改</a></c:otherwise></c:choose>	 | <a href="<%=basePath %>admin/core/res/o_delete_single.do?root=${root}&name=%2F${file.filename}" onclick="if(!confirm('您确定删除吗？')) {return false;}" class="pn-opt">删除</a></td>
</tr>
</c:forEach>
</tbody>
</table>
