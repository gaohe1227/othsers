package com.doadway.dwcms;

import static com.doadway.common.web.Constants.UTF8;
...
public class Install {
	...
	public static void updateConfig(String dbHost, String dbPort,
		...
		Connection conn = getConn(dbHost, dbPort, dbName, dbUser, dbPassword);
		Statement stat = conn.createStatement();
		String sql = "update dc_site set domain='" + domain + "'";
		stat.executeUpdate(sql);
		sql = "update dc_config set context_path='" + cxtPath + "',port="
				+ port;
		stat.executeUpdate(sql);
		stat.close();
		conn.close();
