...
public class AcquisitionSvcImpl implements AcquisitionSvc {
	...
	public boolean start(Integer id) {
		Acquisition acqu = acquisitionBiz.queryAcquisitionById(id);
	...
	private class AcquisitionThread extends Thread {
		...
		public AcquisitionThread(Acquisition acqu) {
			super(acqu.getClass().getName() + "#" + acqu.getAcquisitionId());
			this.acqu = acqu;
		}
		...
		public void run() {
			...
			acqu = acquisitionBiz.startAcquisitionById(acqu.getAcquisitionId());
			...
			for (int i = plans.length - currNum; i >= 0; i--) {
				...
				for (int j = contentList.size() - currItem; j >= 0; j--) {
					if (acquisitionBiz.isNeedBreak(acqu.getAcquisitionId(),
							plans.length - i, contentList.size() - j,
							contentList.size())) {
						client.getConnectionManager().shutdown();
						log.info("Acquisition#{} breaked", acqu.getAcquisitionId());
						return;
					}
			...
			client.getConnectionManager().shutdown();
			acquisitionBiz.endAcquisitionById(acqu.getAcquisitionId());
			log.info("Acquisition#{} complete", acqu.getAcquisitionId());
		}
		...
		private ContentDTO saveContent(HttpClient client, CharsetHandler handler,
			...
			try {
				...
				txt=HtmlUtils.delScriptAndStyle(txt);
				return acquisitionBiz.saveContent(title, txt, acquId);
			} catch (Exception e) {
				log.warn(null, e);
				return null;
			}
