...
public class LuceneContent {
	 ...
	 * @return
	 */
	public  Document createDocument(ContentDTO c) {
		Document doc = new Document();
		doc.add(new Field(ID, c.getContentId().toString(), Field.Store.YES,
				Field.Index.NOT_ANALYZED));
		doc.add(new Field(SITE_ID, c.getSiteId().toString(),
				Field.Store.NO, Field.Index.NOT_ANALYZED));
		doc.add(new Field(RELEASE_DATE, DateTools.dateToString(c
				.getReleaseDate()==null?new Date(System.currentTimeMillis()):c
						.getReleaseDate(), Resolution.DAY), Field.Store.NO,
				Field.Index.NOT_ANALYZED));
		ChannelDTO channel = channelBiz.findChannelById(c.getChannelId());
		while (channel != null) {
			doc.add(new Field(CHANNEL_ID_ARRAY, channel.getChannelId().toString(),
					Field.Store.NO, Field.Index.NOT_ANALYZED));
			if(channelBiz.findChannelById(channel.getParentId())!=null){
				channel = channelBiz.findChannelById(channel.getParentId());
			}else{
				channel=null;
			}
		}
