...
public class HqlFilter {
	...
	private String getSqlOperator(String operator) {
		...
		if (StringUtils.equalsIgnoreCase(operator, "NIS")) {
			return " is not ";
		}
	 ...
	 * @param params
	 */
	public void addFilterSql(String name, String value) {
		if (name != null && value != null && value.length() > 0) {
			if (name.startsWith("QUERY_")) {// 如果有需要过滤的字段
				String[] filterParams = StringUtils.split(name, "_");
				if (filterParams.length == 4) {
					...
					sql.append(" and " + columnName + " " + getSqlOperator(operator) + " " + getObjValueSql(columnType, operator, value) + " ");// 拼SQL
	...
	/**
	 * 将String值转换成Object，用于拼写SQL，替换操作符和值
	 * 
	 * S:String L:Long I:Integer D:Date ST:Short BD:BigDecimal FT:Float
	 * 
	 * @param columnType
	 ...
	 * @return
	 */
	private Object getObjValueSql(String columnType, String operator, String value) {
